 ...
 * Dawn chorus.
 *
 * Acoustic model: songbird syrinx output is close to a pure, rapidly swept
 * tone, so each chirp is a 30–120 ms sine sweep in the 2–6 kHz band under a
 * raised-cosine envelope (no clicks, no harshness). Three chirp shapes cover
 * most passerine vocabulary: up-sweeps, down-sweeps and trills (a fast 17–28
 * Hz alternation between two neighbouring pitches). Chirps are grouped into
 * phrases of 3–9, and — the real signature of birdsong — each voice owns a
 * small repertoire of 2–3 phrase patterns generated from its seed, which it
 * repeats with variation rather than improvising freshly each time.
 * "variety" controls how far each repetition mutates (pitch, duration, chirp
 * count, occasionally a substituted chirp shape).
 *
 * Voices are panned across the field; the more distant ones are quieter and
 * lowpassed. "activity" raises both the voice count (2–4) and the phrase rate
 * by compressing the 1–6 s gaps between phrases. Bright and alive but never
 * shrill — this doubles as the wake-up alarm bed.
 *
 * Params: "activity" 0..1 (default 0.5), "variety" 0..1 (default 0.5).
 */
class BirdsongElement : SoundGenerator {
    ...
    private inner class Bird(seed: Int) {
        private val r = Random(seed)
        private val patterns = 2 + r.nextInt(2)

        // Repertoire (generated once, never mutated).
        private val pLen = IntArray(MAX_PAT)
        private val pType = Array(MAX_PAT) { IntArray(MAX_CHIRP) }
        private val pA = Array(MAX_PAT) { FloatArray(MAX_CHIRP) }
        private val pB = Array(MAX_PAT) { FloatArray(MAX_CHIRP) }
        private val pDur = Array(MAX_PAT) { FloatArray(MAX_CHIRP) }
        ...
        private val cA = FloatArray(MAX_CHIRP)
        private val cB = FloatArray(MAX_CHIRP)
        ...
        init {
            ...
            for (p in 0 until patterns) {
                ...
                for (c in 0 until n) {
                    val t = r.nextInt(3)
                    pType[p][c] = t
                    val f = (centre * (0.82f + r.nextFloat() * 0.42f)).coerceIn(2000f, 5200f)
                    val span = 0.15f + r.nextFloat() * 0.50f
                    when (t) {
                        0 -> {
                            pA[p][c] = f
                            pB[p][c] = (f * (1f + span)).coerceAtMost(6000f)
                        }
                        1 -> {
                            pA[p][c] = (f * (1f + span)).coerceAtMost(6000f)
                            pB[p][c] = f
                        }
                        else -> {
                            pA[p][c] = f
                            pB[p][c] = (f * (1.10f + 0.18f * r.nextFloat())).coerceAtMost(6000f)
                        }
                    }
                    pDur[p][c] = 0.032f + r.nextFloat() * 0.085f
                    pGap[p][c] = 0.028f + r.nextFloat() * 0.100f
                }
            }
            val near = r.nextFloat()
            lp.lowpass(5200f + 6500f * near)
            amp = 0.20f + 0.12f * near
            val a = (0.12f + r.nextFloat() * 0.76f) * PAN_Q
            gainL = cos(a)
            gainR = sin(a)
        ...
        private fun startChirp() {
            chirpPos = 0
            val n = cDur[chirpIdx]
            freq = cA[chirpIdx].toDouble()
            if (cType[chirpIdx] == 2) {
                trillPhase = 0.0
                trillInc = TAU * (17.0 + r.nextDouble() * 11.0) / SAMPLE_RATE
                freqMul = 1.0
            } else {
                freqMul = (cB[chirpIdx].toDouble() / cA[chirpIdx].toDouble()).pow(1.0 / n)
            }
        }
        ...
        private fun startPhrase(v: Float, act: Float) {
            ...
            if (r.nextFloat() < 0.25f * v) {
                ...
                n = (n + step).coerceIn(3, MAX_CHIRP)
            }
            cLen = n
            for (c in 0 until n) {
                val k = if (c < src) c else src - 1
                var t = pType[patIdx][k]
                if (r.nextFloat() < 0.18f * v) t = r.nextInt(3)
                cType[c] = t
                val jf = 1f + 0.12f * v * r.nz()
                cA[c] = (pA[patIdx][k] * jf).coerceIn(1900f, 6200f)
                cB[c] = (pB[patIdx][k] * jf).coerceIn(1900f, 6200f)
                cDur[c] = (pDur[patIdx][k] * (1f + 0.30f * v * r.nz()) * SAMPLE_RATE)
                    .toInt().coerceIn(1320, 5300)
                cGap[c] = (pGap[patIdx][k] * (1f + 0.40f * v * r.nz()) * SAMPLE_RATE)
                    .toInt().coerceIn(400, 12000)
            }
            chirpIdx = 0
            startChirp()
            // Longer phrases when the chorus is busy get a touch less spacing.
            if (act > 0.9f) cGap[0] = (cGap[0] * 0.85f).toInt().coerceAtLeast(400)
        }

        fun sample(out: FloatArray, act: Float, v: Float) {
            var raw = 0f
            if (singing) {
                val n = cDur[chirpIdx]
                if (chirpPos < n) {
                    val e = 0.5f - 0.5f * cos(TAU * chirpPos / n).toFloat()
                    val f: Double
                    if (cType[chirpIdx] == 2) {
                        trillPhase += trillInc
                        if (trillPhase > TAU) trillPhase -= TAU
                        val m = 0.5 + 0.5 * sin(trillPhase)
                        f = cA[chirpIdx] + (cB[chirpIdx] - cA[chirpIdx]) * m
                    } else {
                        freq *= freqMul
                        f = freq
                    }
    ...
    private val birds = Array(MAX_BIRDS) { Bird(7700 + it * 311) }
    private val acc = FloatArray(2)

    override fun render(left: FloatArray, right: FloatArray, frames: Int) {
        val act = activity
        val v = variety
        val n = (2 + (act * 2f).roundToInt()).coerceIn(2, MAX_BIRDS)
        for (i in 0 until frames) {
            acc[0] = 0f
            acc[1] = 0f
            for (b in 0 until n) birds[b].sample(acc, act, v)
            left[i] = softLimit(acc[0])
            right[i] = softLimit(acc[1])
        }
    }
    ...
    private companion object {
        const val MAX_BIRDS = 4
        const val MAX_PAT = 3
        const val MAX_CHIRP = 9
    }
}
